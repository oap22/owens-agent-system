...
To roll back, run `python3 scripts/setup.py --rollback ~/.local/state/owens-agent-system/backups/<run>` to preview, then add `--apply` to execute. It restores an original or removes an installer-created file only while its SHA-256 matches the installed hash. Modified files are kept, including preexisting files. Rollback rechecks its complete plan before writes and restores recorded modes. The backup directory is never deleted. This installer does not uninstall CLI packages.
...
`doctor MODE --workspace PATH --model ID --lead-effort LEVEL` uses that selected executable and the same workspace/configuration as launch. It also accepts `--worker-model`, `--worker-effort`, and `--worker-retry-effort` and creates the immutable role snapshots to validate. Native client startup and account eligibility remain separate checks.
