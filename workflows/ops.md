...
1. Verify connected account and workspace identity before reading or acting. OWE/owenp22/Agent Work and RES are distinct. Missing access is a blocker, never permission to use another workspace.
2. Read live calendar and task state for the requested period. State coverage gaps. Preserve explicit deadlines and time zones; never infer a deadline from urgency language.
3. Focus the daily plan on one outcome, real commitments, available time, and conflicts. Use existing daily-note ownership markers. Avoid rebuilding a dashboard that already exists.
